...
-- Проверка на корректность загрузки
	...
	CREATE TABLE #ProcessedRows(ActionType varchar(255), ID int) -- 9 нет удаления временных таблиц
	-- 10 таблица создается, но нигде не используется

	-- 11 нет проверки существования временной таблицы
	...
	into #CustomerSeasonal -- 12 нет обработки ошибки, если придет ошибка, то сообщения не будет
	from syn.SA_CustomerSeasonal cs -- 13 неоднозначное использование алиасов
		join dbo.Customer as cc on cc.UID_DS = sa.UID_DS_Customer -- 14 неообнозначное использование алиасов
			and cc.ID_mapping_DataSource = 1
		join dbo.Season as s on s.Name = sa.Season -- 15 неявное указание типо join
		join dbo.Customer as cd on cd.UID_DS = sa.UID_DS_CustomerDistributor -- 16 неявное указание типо join
	...
	select
		...
		,case
			...
			when try_cast(sa.DateEnd as date) is null then 'Невозможно определить Дату начала' -- 17 должно быть 'Невозможно определить Дату окончания'
